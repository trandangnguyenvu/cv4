...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- trang này hiển thị cấc đợt QG có lượt QG đang chờ xác nhận--%>
...
<div id="layoutSidenav_content"> <!-- CLOSE TAG OF THIS DIV TAG IS AT FOOTER.JSP -->
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">DS các đợt Quyên góp có lượt QG đang chờ xác nhận</h1>
                    ...
                    <div class="card mb-4">
                        <div class="card-body">                       
                            <table>                               
                                ...
                                <tr>
                                	<th>Tổng số đợt quyên góp có lượt quyên góp chưa được xác nhận <i>* * (DS chi tiết ở bên dưới) * *</i></th>
                                    ...
                                    <td style="text-align: right;">
                                		...
                                		List<DonorDetail> listTempPending = new ArrayList<DonorDetail>();
                                    	for (DonorDetail d: ddList) {                                    		
                                    		listTempPending.add(d);                                    		
                                    	}
                                    	session.setAttribute("listTempPending", listTempPending);
                                    	
                                    	// duyệt trong List, cái nào có money = 0 và status không phải là 'Pending' thì xóa đi                                    	
                                    	for (int i = 0; i < listTempPending.size(); i++) {
                                    		if (listTempPending.get(i).getMoney() == 0 || !listTempPending.get(i).getStatus().equals("Pending") ) {
                                    			listTempPending.remove(i);
                                    			i--;
                                    		}
                                    	}
                                    	...
                                    	for (int i = 0; i < listTempPending.size(); i++) { //'i < listTempPending.size() - 1;' hoặc 'i < listTempPending.size();' đều được'
                                    		for (int j = i + 1; j < listTempPending.size(); j++) {
                                    			if (listTempPending.get(j).getdID().equals(listTempPending.get(i).getdID()) ) {
                                    				listTempPending.remove(j);
                                    				j--;
                                    			}
                                    		}
                                    	}
                                    	...
                                    	int sizeOfListTempPending = listTempPending.size();
                                    	out.print(sizeOfListTempPending);                                    	
                                		%>                               		
                                	</td>
                                    <td style="width: 12px;"></td>
                                    <td>đợt</td>
					...
					<div class="card mb-4"><%-- div mother of all table --%>
                        <div class="card-header">
                            ...
                            Danh sách các đợt Quyên góp có lượt QG chưa được xác nhận
                        </div>
                        <div class="card-body">
                            <table id="datatablesSimple">
								...
								<c:forEach var="donor" items="${listTempPending }">
								<%-- không thêm bất cứ code nào can thiệp vào list (object) 'listTempPending' ngoài những code tại mục ‘Tổng số đợt quyên góp có lượt quyên góp chưa được xác nhận’, nếu không thì giá trị của list này sẽ thay đổi --%>										
										...
										<tr>
											...
											<td><a href="/QuyTuThienVanTinh/Controller?action=detaildonorpending&iddonationround=${donor.getdID()}">Detail</a></td>									

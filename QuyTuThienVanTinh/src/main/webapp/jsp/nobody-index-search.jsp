...
<div id="layoutSidenav_content"><%-- close div of this open div at footer jsp --%>
			...
			String search = request.getParameter("characters");
			// Donation_Round table with pagination
			List<DonationRound> list = ListOfDonationRoundDAO.getRecords4PaginationSearch(offset, total, search);  
			...
			*/
			...
			int countDonations = a.countDonationsSearch(search);
			int totalOfPages = 0;
			if (countDonations < 6) {
				totalOfPages = 1;
			} else {
			...
			session.setAttribute("countDonations", countDonations);
			...
			%>
 									...
 									<h5>Tìm được: ${countDonations } kết quả</h5><br>
            						...
            						<div style="text-align: center;">
	            						<% for (int i = 1; i < totalOfPages; i++) { 
	            							out.print("<a href='/QuyTuThienVanTinh/Controller?action=search&page="+ i + "&characters=" + search + "'>"+ i +"</a><span> | </span>");
	            						}
	            						out.print("<a href='/QuyTuThienVanTinh/Controller?action=search&page="+ totalOfPages + "&characters=" + search + "'>"+ totalOfPages +"</a>");
	            						%>

			...
			*/
			...
			for (DonationRound e:list) {
				...
				for (DonorDetail d:ddList) {
					if(Integer.toString(e.getId()).equals(d.getdID()) && d.getMoney() > 0 && d.getStatus().equals("Confirmed")) {
			...
			%>
 									...
 									<h5>Đã quyên góp được tổng cộng: 
 										<%
                                    	float moneyTotal = 0;
                                    	for (DonorDetail d: ddList) {
 									...
 									<h5>Có ${countDonations } dự án đã và đang triển khai</h5><br>
            								...
            								<%-- <c:if test="${now.after(donation.getStartDate()) || now.equals(donation.getStartDate()) }"> not filter at here cause filtered at DAO--%> 
						                        <div class="col-xl-3 col-md-6">
						                            <div class="card bg-primary text-white mb-4">
						                                <div ><%-- class="card-body" --%>
						                                <a href="/QuyTuThienVanTinh/Controller?action=one&id=${donation.getId() }">
						                                <img class="st-img"
							                               		...
							                               		<table style="width:100%"><%-- để nút Quyên at right => tạo 5 table nối tiếp: 1 chứa Lượt QG, 2 trống, 3 chứa Đạt được, 4 trống, 5 chứa nút Quyên góp --%>
							                                        <tr>
							                                            <td colspan="3" style="padding-bottom: 20px;">							                                            	
																			...
																			for (DonorDetail d:ddList) {
																				if (session.getAttribute("totalMoneyEachDonation").equals(d.getdID()) && d.getMoney() > 0 && d.getStatus().equals("Confirmed")) {
